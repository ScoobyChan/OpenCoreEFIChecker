...
class OC:
	def __init__(self):
		...
		self.UEFI = {"Protocols":{"AppleBootPolicy":False,"AppleEvent":False,"AppleImageConversion":False,"AppleKeyMap":False,"AppleUserInterfaceTheme":False,"ConsoleControl":True,"DataHub":False,"DeviceProperties":False,"FirmwareVolume":False,"HashServices":False,"UnicodeCollation":False}, "Quirks":{"AvoidHighAlloc":False,"ExitBootServicesDelay":0,"IgnoreInvalidFlexRatio":False,"IgnoreTextInGraphics":False,"ProvideConsoleGop":True,"ReleaseUsbOwnership":False,"RequestBootVarRouting":True,"ReplaceTabWithSpace":False,"SanitiseClearScreen":True,"ClearScreenOnModeSwitch":False}}
		self.BOOTER = {"Quirks":{"AvoidRuntimeDefrag":True,"DevirtualiseMmio":False,"DisableVariableWrite":False,"DisableSingleUser":False,"DiscardHibernateMap":False,"EnableSafeModeSlide":True,"EnableWriteUnprotector":True,"ForceExitBootServices":False,"ProtectCsmRegion":False,"ProvideCustomSlide":True,"SetupVirtualMap":True,"ShrinkMemoryMap":False}}
		self.KERNEL = {"Quirks":{"AppleCpuPmCfgLock":False,"AppleXcpmCfgLock":False,"AppleXcpmExtrasMsrs":False,"CustomSMBIOSGuid":False,"DisableIOMapper":False,"ExternalDiskIcons":True,"LapicKernelPanic":False,"PanicNoKextDump":True,"PowerTimeoutKernelPanic":True,"ThirdPartyDrives":False,"XhciPortLimit":True}}
		self.MISC = {"Boot":{"HideSelf":True,"PollAppleHotKeys":False,"ShowPicker":True},"Debug":{"DisableWatchDog":False},"Security":{"AllowNvramReset":True,"RequireSignature":False,"RequireVault":False,"ScanPolicy":0}}
		self.NVRAM = {"LegacyEnable":False}
		self.PLATINFO = {"Automatic":True,"Generic":{"MLB":"M000000000001","SpoofVendor":False,"SystemSerialNumber":"W0000000001","SystemUUID":"00000000-0000-0000-0000-000000000000"},"UpdateDataHub":True,"UpdateNVRAM":True,"UpdateSMBIOS":True,"UpdateSMBIOSMode":"Create"}
		self.KEXTS = ['WhateverGreen.kext', 'Lilu.kext', 'NullCPUPowerManagement.kext', 'VirtualSMC.kext','AppleMCEReporterDisabler.kext']
		# Needed For iMacPro
		self.OCStructure = {"ACPI":{"Add":None,"Block":None,"Patch":None,"Quirks":['FadtEnableReset','NormalizeHeaders','RebaseRegions','ResetHwSig','ResetLogoStatus']},"Booter":{"MmioWhitelist":None,"Quirks":["AvoidRuntimeDefrag","DevirtualiseMmio","DisableSingleUser","DisableVariableWrite","DiscardHibernateMap","EnableSafeModeSlide","EnableWriteUnprotector","ForceExitBootServices","ProtectCsmRegion","ProvideCustomSlide","SetupVirtualMap","ShrinkMemoryMap"]},"DeviceProperties":{"Add":None,"Block":None},"Kernel":{"Add":None,"Block":None,"Emulate":None,"Patch":None,"Quirks":["AppleCpuPmCfgLock","AppleXcpmCfgLock","AppleXcpmExtraMsrs","CustomSMBIOSGuid","DisableIoMapper","ExternalDiskIcons","LapicKernelPanic","PanicNoKextDump","PowerTimeoutKernelPanic","ThirdPartyDrives","XhciPortLimit"]},"Misc":{"BlessOverride":None,"Boot":['ConsoleBehaviourOs','ConsoleBehaviourUi','ConsoleMode','HibernateMode','HideSelf','PollAppleHotKeys','Resolution','ShowPicker','Timeout','UsePicker'],"Debug":['DisableWatchDog','DisplayDelay','DisplayLevel','Target'],"Entries":None,"Security":['AllowNvramReset','ExposeSensitiveData','HaltLevel','RequireSignature','RequireVault','ScanPolicy'],"Tools":None},"NVRAM":{"Add":None,"Block":None,"LegacyEnable":None,"LegacySchema":None},"PlatformInfo":{"Automatic":None,"Generic":["MLB","ROM","SpoofVendor","SystemProductName","SystemSerialNumber","SystemUUID"],"UpdateDataHub":None,"UpdateNVRAM":None,"UpdateSMBIOS":None,"UpdateSMBIOSMode":None},"UEFI":{"ConnectDrivers":None,"Drivers":None,"Input":['KeyForgetThreshold','KeyMergeThreshold','KeySupport','KeySupportMode','KeySwap','PointerSupport','PointerSupportMode','TimerResolution'],"Protocols":['AppleBootPolicy','AppleEvent','AppleImageConversion','AppleKeyMap','AppleUserInterfaceTheme','ConsoleControl','DataHub','DeviceProperties','FirmwareVolume','HashServices','UnicodeCollation'],"Quirks":['AvoidHighAlloc','ClearScreenOnModeSwitch','ExitBootServicesDelay','IgnoreInvalidFlexRatio','IgnoreTextInGraphics','ProvideConsoleGop','ReconnectOnResChange','ReleaseUsbOwnership','ReplaceTabWithSpace','RequestBootVarFallback','RequestBootVarRouting','SanitiseClearScreen','UnblockFsConnect']}}
	...
	def loadConfig(self):
		...
		print("Loaded Config")
		# if '##WARNING - 1' in self.config:
		# 	print("Remove #WARNING - 1")

		# if '##WARNING - 2' in self.config:
		# 	print("Remove #WARNING - 2")

		# if '##WARNING - 3' in self.config:
		# 	print("Remove #WARNING - 3")
	...
	def checkDriversFolder(self):
		...
		defaults = ['hfsplus.efi or vboxplus.efi', 'apfsdriverloader.efi', 'fwruntimeservices.efi']
		for d in self.DF:
			time.sleep(5)
			print(f' - found {d}')
			if d.lower() == 'hfsplus.efi' or d.lower() == 'vboxplus.efi': 
				defaults.remove('hfsplus.efi or vboxplus.efi')
			if d.lower() == 'fwruntimeservices.efi': defaults.remove(d.lower())
			if d.lower() == 'apfsdriverloader.efi': defaults.remove(d.lower())

		if defaults != []:
			self.ERRORS += 'Missing Drivers'
			for d in defaults:
				self.ERRORS += f"\n - {d}"
				...
				if 'LegacyEnable' == c:
					try:
						if self.NVRAM[c] != self.config[check][c]:
							E += f"\n - {q} needs to be set to {self.NVRAM[c][q]}"
					except KeyError:
						pass
		if not E == '':
			self.ERRORS += '\nNVRAM Error'
				...
				if 'UpdateSMBIOS' in c or 'UpdateNVRAM' in c or 'UpdateDataHub' in c or 'Automatic' in c or 'UpdateSMBIOSMode' in c:
					if self.config[check][c] != self.PLATINFO[c]:
						if self.PLATINFO[c] != self.config[check][c]:
							P += f" - \n{q} needs to be set to {self.PLATINFO[c][q]}"
